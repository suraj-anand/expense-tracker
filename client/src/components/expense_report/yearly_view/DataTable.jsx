...
const DataTable = ({ activeYear }) => {
    ...
    <>
        {
            ( !_.isEmpty(parsedExpense) && activeYear ) && 
            ( Object.keys(parsedExpense).includes(activeYear) ) && 
            parsedExpense[activeYear] && 
            ...
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody className="align-middle">
                      {
                        Array.from(Object.keys(parsedExpense[activeYear])).map(month => {
                          return parsedExpense[activeYear][month].map((exp, index) => {
...
function UpdateButton({exp}){
    ...
    return (
        <>
        <UpdateForm expense={exp} modalId={`yearly-expense-update-${id}`} />
        <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target={`#yearly-expense-update-${id}`}>
            <RxPencil2 size={24} />
        </button>
        </>
    )    
